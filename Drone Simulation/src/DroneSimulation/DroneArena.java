...
public class DroneArena {
	 ...
	 * x: the size of the horizontal line
	 * y: the size of the vertical line*/
...
/** a method to add a new drone to 
 * the array list manyDrones
 * */
public void addDrone() {
	...
	do { // a do while loop that adds drones in a random positions
	 a1 = a.nextInt(xmax-1);
	 a2 = a.nextInt(ymax-1);
	} while((getDroneAt(a1,a2) != null) || (a1 == 0) || (a1 == xmax - 1) || (a2 == 0) || (a2 == ymax - 1)); // this condition make sure the drone positions are within the arena
	
	b = new Drone ( a1 ,  a2, z); // creating the new drone
	manyDrones.add(b); // adding the new drone in the array list
}
/**a method to check if the positions of the new
 * drones that will be added wont be the same as the old drones
 ...
 * the method that moves all the drones in their direction*/
 void moveAllDrones() {
	for (int i=0; i < manyDrones.size(); i++) { // a for loop to go through each drone in the array list and move it using the tryToMove method
    	manyDrones.get(i).tryToMove(this);
    	}
	}
 /** a method to show the drones using displayDrone method
  * and the input would be a ConsoleCanvas object c
  * */
public void showDrones(ConsoleCanvas c) {
	for (int i=0; i <manyDrones.size(); i++) { //a for loop to go through each drone in the array list and display it 
    	manyDrones.get(i).displayDrone(c);
    	}
}

/**a method to check if the drone can move to the next position
 * using getDroneAt method
 * x: the horizontal position
 * y: the vertical position
 * */
public boolean canMoveHere(int x, int y){
    if( x < this.xmax-1 && y < this.ymax-1 && x != 0 && y != 0 && getDroneAt(x,y) ==  null){ //if within the border of the arena and no drone is there
       return true;
    }else{
       return false;
    }
