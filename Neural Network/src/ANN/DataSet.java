...
/**
 * 
 * Class for data set of logic values, for neural nets Each item in set has
 * inputs, targets and outputs
 */
public class DataSet {
	/**
	 * has variables for number of inputs, outputs and items in the data set and
	 * then an array list of items each item is in a private class which has inputs,
	 * output and targets
	 */
	...
	protected class DataItem {
		...
		public DataItem(int numIns, String data) {
			...
			for (int ct = 0; ct < dataSplit.length; ct++) { // for each
				...
				if (ct < numIns)
					dataInputs.add(d); // if is input, add to dataInputs
				else {
	 ...
	 * 
	 * @param alldata string defining all data alldata has series of strings for
	 *                each item separated by ; first string has numIns numOuts, rest
	 *                are for each item second string has names of input and output
	 *                variables string for an item has numbers for inputs then
	...
	public DataSet(String alldata) {
		...
		if (fLine.length > 2)
			...
			inFormat = "%.1f"; // get format for printing ins
		if (fLine.length > 3)
			outFormat = fLine[3];
		else
			outFormat = "%.1f"; // and for outputs
		if (fLine.length > 4)
			rawFormat = fLine[4];
		else
			rawFormat = "%.2f"; // and for raw outputs
	...
	private void setNames(String[] names) {
		for (int ct = 0; ct < names.length; ct++)
			if (ct < numIns)
				inNames.add(names[ct]);
			else
				outNames.add(names[ct]);
	}
	...
	private String fixedWidth(String s, int mnum) {
		String ans = "";
		for (int ct = 0; ct < mnum - s.length(); ct++)
			ans = ans + ' ';
		ans = ans + s;
		return ans;
	}
	...
	private String namesToString(ArrayList<String> names) {
		String s = "";
		for (int ct = 0; ct < names.size(); ct++)
			s = s + fixedWidth(names.get(ct));
		return s;
	}

	/**
	 * Function to convert data into string; for each item output inputs, targets,
	 * and if req outputs
	 * 
	...
	public String toString(boolean rawOuts, boolean withOuts) {
		...
		if (rawOuts)
			s = s + fixedWidthCentre("Raw Ops", numOutputs());
		if (withOuts)
			s = s + fixedWidthCentre("Outputs", numOutputs());
		s = s + '\n';
		...
		for (ct = 0; ct < numInSet(); ct++) { // now print each item in the set
			...
			if (rawOuts)
				s = s + valstoString(getOuts(ct), rawFormat);
			if (withOuts)
				s = s + valstoString(getPrintOuts(ct), outFormat);
			s = s + "\n";
		}
		return s;
	}
	...
	/**
	 * calculate the sum of the squares of errors for each output across the data
	 * set
	 * 
	...
	public ArrayList<Double> getSSE() {
		...
		for (int ct = 0; ct < numOuts; ct++)
			SSE.add(0.0); // for each output set SSE to 0
		for (int item = 0; item < numInSet(); item++) { // for each item in the set
			...
			for (int ct = 0; ct < numOuts; ct++)
				SSE.set(ct, SSE.get(ct) + errs.get(ct) * errs.get(ct));
			// add square of each error to total
		}
		for (int ct = 0; ct < numOuts; ct++)
			SSE.set(ct, SSE.get(ct) / numInSet());
	...
	/**
	 * calculate the number of outputs which are correctly classified (ie target =
	 * output)
	 * 
	...
	public ArrayList<Integer> getCorrect() {
		...
		for (int ct = 0; ct < numOuts; ct++)
			Correct.add(0); // for each output, set sum to 0
		for (int item = 0; item < numInSet(); item++) { // for each item in the set
	...
	/**
	 * generate analysis of results, return sum of square of errors SSE and if a
	 * classification set, % of outputs correctly classified
	 * 
	...
	public String dataAnalysis() {
		...
		String s = "SSE ";
		for (int ct = 0; ct < numOuts; ct++)
			s = s + String.format("%.4f", SSE.get(ct)) + " ";
	...
	/**
	 * Calculate the SSE following the lastest presentation of dataset, and add to
	 * the log
	 */
	...
	public ArrayList<Double> getSSELog(int whichOut) {
		...
		for (int ct = 0; ct < sizeSSELog(); ct++)
			ans.add(sseLog.get(ct).get(whichOut));
