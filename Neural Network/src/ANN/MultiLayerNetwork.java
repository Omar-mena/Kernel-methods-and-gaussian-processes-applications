...
/**
 * This a multi layer network, comprising a hidden layer of neurons with sigmoid
 * activation Followed by another layer with linear/sigmoid activation, or be
 * another multi layer network A layer is defined as a set of neurons which have
 * the same inputs
 */
public class MultiLayerNetwork extends SigmoidLayerNetwork {
	...
	protected void calcOutputs(ArrayList<Double> nInputs) {
		...
		nextLayer.calcOutputs(getOutputs());
	}
	...
	protected void outputsToDataSet(int ct, DataSet d) {
		...
		d.setOutputs(ct, nextLayer.outputs);
	}
	...
	protected void findDeltas(ArrayList<Double> errors) {// this function sets the delta using linear activation
	...
	protected void changeAllWeights(ArrayList<Double> ins, double learnRate, double momentum) {
		// loops through up to the number of neurons.
	...
	public String getWeights() {
		String s = ""; // Sets the empty string called 's'
		for (int ct = 0; ct < numWeights; ct++) // loops through the weights
			s = s + String.format("%.5f", weights.get(ct)) + " "; // adds the contents to s upto 5 decimal points

		for (int ct = 0; ct < nextLayer.numWeights; ct++) // loops through the weights
			s = s + String.format("%.5f", weights.get(ct)) + " "; // adds the contents to s upto 5 decimal points
		return s; // returns the string result
	}
...
//		System.out.println(MLN.doLearn(2000, 0.4,  0.7));
	...
	public static void TestThree() {
		...
		MultiLayerNetwork MLN = new MultiLayerNetwork(2, 4, Other,
				new MultiLayerNetwork(4, 3, Other, new SigmoidLayerNetwork(3, 2, Other)));
		MLN.presentDataSet(Other);
		MLN.doInitialise();
		System.out.println(MLN.doPresent());
		System.out.println("Weights " + MLN.getWeights());
